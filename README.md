...
- **2026-04-25**: **Codex Obsidian KB CRUD stabilized** — repaired the Codex project KB lifecycle so rename keeps stable registry IDs, archive moves notes only into the Archive section, purge removes registry/index/link residue, and lint reports `broken_links = 0`; Codex runtime bindings now stay in `.codex/project-memory/*` with JSON content in the historical `registry.yaml` file.
- **2026-04-24**: **Vault-first Obsidian KB workflow backported to Codex** — consolidated project memory into `obsidian-project-kb-core`, `obsidian-source-ingestion`, `obsidian-literature-workflow`, and `obsidian-kb-artifacts`; moved paper sources under `Sources/Papers/`; preserved `.codex/project-memory/*` as a runtime binding layer; and made `02-Index.md` a human-readable navigation page rather than a registry mirror.
